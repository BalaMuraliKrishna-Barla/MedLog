...
const sendAppointmentReminderEmail = (user, appointment) => {
    ...
    const mailOptions = {
        ...
        to: user.email,
        subject: `📅 Appointment Reminder: ${appointment.purpose} on ${new Date(appointment.appointmentDateTime).toLocaleDateString()}`,
        html: `
            ...
            <div style="max-width: 600px; margin: auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
                ...
                <div style="padding: 30px;">
                    ...
                    <table style="width: 100%; margin-top: 20px; border-collapse: collapse; background-color: #fafafa; border-radius: 8px; overflow: hidden;">
                        <tr style="border-bottom: 1px solid #e0e0e0;"><td style="padding: 12px 15px;">👨‍⚕️ <strong>Doctor</strong></td><td style="padding: 12px 15px;">${appointment.doctorName}</td></tr>
                        <tr style="border-bottom: 1px solid #e0e0e0;"><td style="padding: 12px 15px;">📝 <strong>Purpose</strong></td><td style="padding: 12px 15px;">${appointment.purpose}</td></tr>
                        ...
                        <tr><td style="padding: 12px 15px;">📍 <strong>Location</strong></td><td style="padding: 12px 15px;">${appointment.location || 'N/A'}</td></tr>
                    </table>
    ...
    transporter.sendMail(mailOptions, (error) => {
        if (error) console.error(`Error sending appointment email to ${user.email}:`, error);
        else console.log(`Appointment reminder email sent successfully to ${user.email}`);
    });
};
...
const sendMedicationReminderEmail = (user, medications) => {
  const medicationListHtml = medications.ymap(med => `
        <tr>
            ...
            <td style="padding: 10px 15px; border-bottom: 1px solid #eee;">⏰ ${med.frequency.timings.join(', ')} (${med.instructions})</td>
        </tr>`).join("");
  ...
  transporter.sendMail(mailOptions, (error) => {
    if (error) console.error(`Error sending medication email to ${user.email}:`, error);
    else console.log(`Medication reminder email sent successfully to ${user.email}`);
  });
};
...
const startScheduledJobs = () => {
    ...
    cron.schedule(cronSchedule, async () => {
        ...
        try {
            ...
            if (upcomingAppointments.length > 0) {
                console.log(`[Appointments] Found ${upcomingAppointments.length} upcoming appointments.`);
        ...
        try {
            ...
            if (usersWithActiveMeds.length > 0) {
                 console.log(`[Medications] Found ${usersWithActiveMeds.length} user(s) with active medications.`);
    ...
    console.log(`Notification service started. Cron job scheduled to run: ${cronSchedule}.`);
};
